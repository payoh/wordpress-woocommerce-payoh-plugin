...
class WC_Gateway_Payoh_Ideal_Request extends WC_Gateway_Payoh_Request {
	/**
	 * Constructor.
	 * @param WC_Gateway_Payoh_Ideal $gateway
	 */
	public function __construct( $gateway ) {
		$this->gateway    = $gateway;
		$this->notify_url = WC()->api_request_url( 'WC_Gateway_Payoh_Ideal' );
	}
	...
	public function get_request_url( $order) {
		...
		$comment = get_bloginfo( 'name' ) . " - " . sprintf(__('Order #%s by %s %s %s',PAYOH_IDEAL_TEXT_DOMAIN),$order->get_order_number(), $order->billing_last_name,$order->billing_first_name,$order->billing_email);
		$returnUrl = '';
		$params = array(
		...
		WC_Gateway_Payoh_Ideal::log(print_r($params, true));
		
		//Call APi MoneyInIdealInit in correct MODE with the args
		$idealInit = $this->gateway->getDirectkit()->MoneyInIdealInit($params);
		
		WC_Gateway_Payoh_Ideal::log(print_r($idealInit, true));

		global $wpdb;

		$sql = "INSERT INTO `" . $wpdb->prefix . "payoh_wktoken` (`wktoken`, `id_cart`) VALUES (%s, %d) ON DUPLICATE KEY UPDATE wktoken = %s";
